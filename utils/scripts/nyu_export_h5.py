...
# ---------- Preprocessing (optional): resize/center crop ----------
...
def apply_resize_crop(rgb, depth, resize_wh=None, crop_wh=None):
    if resize_wh is not None:
        ...
        depth = cv2.resize(depth, (w, h), interpolation=cv2.INTER_NEAREST)  # Preserve values
...
def depth_to_mm16(depth_m, max_depth=None):
    """
    meter -> millimeter(uint16). Only convert valid (POSITIVE), others to 0.
    If max_depth>0, clip upper bound to that value (m).
    """
    ...
    d_mm = (d * 1000.0).round()
    d_mm = np.clip(d_mm, 0, 65535).astype(np.uint16)  # Can represent up to 65.535m
    return d_mm

def depth_to_jet(depth_m, max_depth=None):
    """
    Jet colormap visualization (BGR). Normalize with min~max (or max_depth) of valid (>0, finite) pixels.
    """
