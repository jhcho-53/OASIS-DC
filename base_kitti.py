...
def _alt_stems_for_rel(rel_stem: str) -> Dict[str, List[str]]:
    ...
    Example:
      rel_stem = "2011_..._image_0000000005_image_02"
      gt stems : [same, "..._groundtruth_depth_0000000005_image_02"]
      sp stems : [same, "..._velodyne_raw_0000000005_image_02"]
      est stems: try several variants (same / _est_ / _groundtruth_depth_ / _velodyne_raw_)
    """
...
def main():
    ...
    for stem, img_p, gt_p, sp_p, est_p in pbar:
        ...
        # solve plain Poisson (with stats)
        ...
        pbar.set_postfix(MAE=f"{mae_sum/n:.4f}", RMSE=f"{rmse_sum/n:.4f}", d1=f"{d1_sum/n:.4f}",
                         t=f"{solver_times[-1]:.3f}s", it=solver_iters[-1], E=("zeros" if use_zeros else "est"))
    ...
    with open(os.path.join(args.outdir, "metrics.csv"), "w", newline="") as f:
        w = csv.DictWriter(f, fieldnames=list(per_image[0].keys()) if per_image else
                           ["id","mae","rmse","delta1","solver","time_sec","cg_iters","n_sparse","E_norm_source"])
        w.writeheader()
    ...
    if args.emode=="precomputed":
        print(f"est used: {est_used_cnt} | est→zeros fallback: {est_fallback_cnt}")
