...
+ Test-time global monotone (quantile) distribution alignment of E01 to sparse DL.
...
Example:
python eval_kitti_root_poisson_quantile.py \
  --root /data/val_selection_cropped \
  --dmax 80.0 \
  --emode precomputed \
  --outdir runs_kitti_poisson_qm \
  --align quantile --qm_plow 0.02 --qm_phigh 0.98 --qm_k 21 \
  --save-pred --save-k 16
"""
...
def _read_depth(path: str) -> np.ndarray:
    ...
    with Image.open(path) as im:
        if im.mode in ["I;16","I;16B","I"]:
            # KITTI는 보통 mm 단위 16-bit PNG
            return (np.array(im, dtype=np.uint16).astype(np.float32) / 1000.0)
        return np.array(im, dtype=np.uint8).astype(np.float32)  # 이미 m 단위 8bit일 수도 있음
...
def _find_with_exts(base_dir: str, rel_stem: str, exts: Tuple[str,...]) -> Optional[str]:
    # try same dir tree and each extension
    for ext in exts:
...
# ====== NEW: global monotone (quantile) alignment ======
def _quantile_match_monotone(
    E01: np.ndarray, DL01: np.ndarray, mask: np.ndarray,
    plow: float = 0.02, phigh: float = 0.98, k: int = 21
) -> np.ndarray:
    """
    Piecewise-linear monotone map T s.t. T(E01)[mask] ≈ DL01[mask] in distribution.
    - Winsorize tails via [plow, phigh]
    - If anchors insufficient or degenerate, returns E01
    """
    m = (mask > 0)
    n = int(m.sum())
    if n < 10 or k < 2 or not np.isfinite(E01[m]).all() or not np.isfinite(DL01[m]).all():
        return E01

    p = np.linspace(plow, phigh, k).astype(np.float64)
    p = np.clip(p, 0.0, 1.0)
    e = E01[m].astype(np.float64); y = DL01[m].astype(np.float64)
    ...
    Qe_u, idx = np.unique(Qe, return_index=True)
    Qd_u = Qd[idx]
    if Qe_u.size < 2:
        return E01

    # piecewise-linear monotone interpolation
    Ehat = np.interp(E01.astype(np.float64), Qe_u, Qd_u, left=Qd_u[0], right=Qd_u[-1])
    Ehat = np.clip(Ehat, 0.0, 1.0).astype(np.float32)
    return Ehat
...
def poisson_fallback_np(sparse_m: np.ndarray, est_m: np.ndarray, m_bool: np.ndarray,
    ...
    """Jacobi fallback: Dirichlet at border + sparse anchors."""
    v = est_m.copy()
    known = m_bool.copy()
...
def build_poisson_from_enorm_with_stats(
    ...
    device_str: str,
    # NEW:
    align_mode: str, qm_plow: float, qm_phigh: float, qm_k: int, min_valid: int
) -> Tuple[torch.Tensor, Dict[str, float]]:
    """
    B=1 tensors expected. Returns (pred_depth_B1HW, stats).
    align_mode: 'quantile' | 'affine' | 'none'
    """
    assert DL.shape[0] == 1, "This helper assumes B=1."
    e  = E01[0,0].detach().cpu().numpy().astype(np.float32)             # [0,1]
    dl = (DL [0,0] / dmax).detach().cpu().numpy().astype(np.float32)    # ~[0,1]
    m  = (ML [0,0].detach().cpu().numpy().astype(np.float32) > 0)

    # orientation by correlation if requested (keep as heuristic)
    ...
    # ----- NEW: distribution alignment -----
    mode_used = "none"
    if align_mode == "quantile" and int(m.sum()) >= max(10, min_valid):
        e_aligned = _quantile_match_monotone(e, dl, m, plow=qm_plow, phigh=qm_phigh, k=qm_k)
        mode_used = "quantile"
    elif align_mode == "affine" and int(m.sum()) >= 10:
        # robust affine fit on anchors only
        ...
        e_aligned = np.clip(a*e + b0, 0.0, 1.0)
        mode_used = "affine"
    else:
        e_aligned = e.copy()
        mode_used = "none"

    # optional mild smoothing (keeps structure)
    if smooth_est:
        e_aligned = _smooth3_reflect_np(e_aligned)

    est_m    = (e_aligned * dmax).astype(np.float32)
    ...
    stats.update({
        ...
        "qm_plow": qm_plow, "qm_phigh": qm_phigh, "qm_k": qm_k,
        "n_anchors": int(m.sum())
    })
...
def _alt_stems_for_rel(rel_stem: str) -> Dict[str, List[str]]:
    """
    From image rel_stem produce alternative stems for GT/SP/EST matching.
    Works for both hierarchical (unchanged stem) and flat 'val_selection' naming.
    """
...
def scan_kitti_pairs(root: str, require_est: bool) -> Tuple[List[Tuple[str,str,str,str,Optional[str]]], Dict[str,int]]:
    """
    Returns:
        pairs: list of (rel_stem, img_path, gt_path, sp_path, est_path_or_None)
        missing_counts: dict {'gt':x, 'sp':y, 'est':z}
    """
    ...
    for rel in img_rels:
        stem = os.path.splitext(rel)[0]  # keep rel dirs if any
...
def main():
    ...
    # alignment options
    ap.add_argument("--align", type=str, default="quantile", choices=["quantile","affine","none"],
                    help="Test-time alignment of E01 to metric anchors (default: quantile).")
    ap.add_argument("--qm_plow", type=float, default=0.02, help="winsor lower quantile for QM")
    ap.add_argument("--qm_phigh", type=float, default=0.98, help="winsor upper quantile for QM")
    ap.add_argument("--qm_k", type=int, default=21, help="#quantile knots for piecewise-linear monotone map")
    ap.add_argument("--min_valid", type=int, default=60, help="min #anchors to run quantile alignment")
    # legacy toggles (still useful with align!=none)
    ap.add_argument("--no-auto-flip", action="store_true")
    ap.add_argument("--no-smooth", action="store_true")
    ap.add_argument("--no-est-affine", action="store_true", help="(only used if --align affine)")
    # output
    ap.add_argument("--outdir", type=str, default="runs_kitti_poisson_qm")
    ...
    # scan pairs (val_selection compatible)
    ...
    pbar = tqdm(pairs, ncols=120, desc="[KITTI][Plain Poisson + QM]")
    for stem, img_p, gt_p, sp_p, est_p in pbar:
        ...
        if args.emode == "precomputed" and est_p is not None:
            raw = _read_depth(est_p)  # may be 2D float from npy/npz or 8/16-bit image
            if raw.ndim == 2:
                if raw.shape != (H,W):
        ...
        # solve Poisson (with stats) after alignment
        P, st = build_poisson_from_enorm_with_stats(
            ...
            align_mode=args.align, qm_plow=args.qm_plow, qm_phigh=args.qm_phigh,
            qm_k=args.qm_k, min_valid=args.min_valid
        )
        ...
        pbar.set_postfix(MAE=f"{mae_sum/n:.4f}", RMSE=f"{rmse_sum/n:.4f}", d1=f"{d1_sum/n:.4f}",
                         ...
                         E=("zeros" if use_zeros else st.get("align_mode","-")))
        ...
        per_image.append({
            ...
            "n_sparse": int((tML>0).sum().item()),
            "align_mode": st.get("align_mode","none"),
            "qm_plow": st.get("qm_plow", None),
            "qm_phigh": st.get("qm_phigh", None),
            "qm_k": st.get("qm_k", None),
        })
    ...
    summary = {
        ...
        "avg_cg_iters": avg_it,
        "align_default": args.align,
        "qm_plow": args.qm_plow, "qm_phigh": args.qm_phigh, "qm_k": args.qm_k,
    ...
    with open(os.path.join(args.outdir, "metrics.csv"), "w", newline="") as f:
        w = csv.DictWriter(f, fieldnames=list(per_image[0].keys()) if per_image else
                           ["id","mae","rmse","delta1","solver","time_sec","cg_iters","n_sparse",
                            "align_mode","qm_plow","qm_phigh","qm_k"])
        w.writeheader()
        for r in per_image: w.writerow(r)

    # print summary
    print("\n=== KITTI Plain Poisson (root-scan) + Quantile Alignment ===")
